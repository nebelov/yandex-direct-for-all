...
Запуск откроет страницу Яндекса, примет ответ на зарегистрированном локальном адресе и выполнит обязательное безопасное чтение. Явный логин рекламодателя обязателен и проверяется до первого сетевого запроса к Директу. Незавершённый сеанс PKCE сохраняется и продолжается повторным запуском той же команды.
