...
def build_draft_text(loser: dict[str, str]) -> tuple[str, str]:
    ...
    if "сегмент-а" in surface:
        return (
            "Пример товара для сегмента А",
            f"Характеристика товара для сегмента А. Подберём размер и доставим {region}.",
        )
    if "сегмент-б" in surface:
        return (
            "Пример товара для сегмента Б",
            f"Подтверждённое преимущество товара. Подбор товара и доставка {region}.",
        )
    if "сегмент-г" in surface:
        return (
            "Пример товара для сегмента Г",
            f"Подтверждённая характеристика товара. Подбор типа, образцы и доставка {region}.",
        )
    if "тип" in surface and "сегмент-б" in campaign_name.lower():
        return (
            "Пример товара для сегмента Б",
            f"Подберём вариант под задачу. Доставка {region}.",
        )
    return (
        "Пример товара для сегмента В",
        f"Подтверждённая характеристика товара. Подбор типа, образцы и доставка {region}.",
    )
